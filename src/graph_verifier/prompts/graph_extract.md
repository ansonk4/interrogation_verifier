...
Return only JSON:
{
  ...
  "edges": [{"id": "e1", "premise_node_ids": ["n1"], "target_node_id": "n2", "claim": "2 + 2 = 4"}],
...
Every root fact or query constraint taken from the question must be its own node and quote only the operative question phrase exactly, including its original math notation. Use kind `query_constraint`, never kind `question`, and omit interrogative filler such as "what is" or "find". Put formalization or interpretation in an outgoing edge, not inside the quoted node. Every question-specific value introduced by an edge must appear in one of that edge's premise nodes.
